...
- **Connection management** — The top-bar "SSH" button opens a management dialog with a two-pane layout (group list on the left, connections of the selected group on the right) to add / edit / delete SSH connections, with host / port / username / password / private key / group fields, persisted to the config file; the password field has a Show / Hide toggle. The "Link SSH" and "Add remote project" dialogs share the same structure (one group-bucketing implementation, collapsible groups in the All view, and select-all / clear-all acting only on currently visible connections), and deleting a connection asks for confirmation first, warning that the stored password and private-key path will be lost.
...
- **Encrypted password storage** — Saved passwords never hit disk in plaintext: they are sealed with AES-256-GCM before being written to `config.db` and to the sidecar-facing `config.json`. The master key lives in `credential.key` in the data directory, wrapped by DPAPI for the current user on Windows and protected by 0600 permissions on macOS / Linux. Plaintext left behind by older versions is sealed once on the first launch after upgrading, and the database file is scrubbed (including the `.bak` and the `.pre-sqlite` archive). Only the main app generates the key; sidecars open it read-only and unseal at authentication time. If a password cannot be decrypted the UI asks you to re-enter it rather than sending the ciphertext as a password.
...
- **Hook event system** — Integrates the official Claude Code / Codex / Grok Build / oh-my-pi Hook APIs to receive AI tool events (SessionStart / End, ToolUse, etc.), which is more precise and timely than process polling; the built-in `miniterm-hook` CLI is called by the hook system to POST events to a local server (oh-my-pi instead loads an in-process extension, see below); the settings UI registers / unregisters hooks per CLI via "injection targets" — one checkbox row each for Claude Code / Codex / Grok / oh-my-pi, with registration and removal acting only on the selected ones (the config files are unrelated; a user of just one CLI has no reason to get the others written). Each row shows that CLI's config file path and registration state (not registered / N events registered / outdated N⁄M in yellow, prompting a re-register to pick up newly added events); the default selection is whichever CLIs are already registered (so an old user hitting register is a pure top-up), falling back to all three when none is, preserving the first-run one-click experience. Writes merge rather than overwrite your existing hooks. Codex permission requests stay in `ai-working` through approval and tool execution, avoiding premature completion notifications.
- **Real-time status detection** — Once hooks are reporting they are the status source for that pane; each polling round reads the hook state directly and never consults output activity (a TUI's idle redraws used to read as "working again," firing the completion notification over and over). Panes without hooks fall back to input detection (recognizing typed `claude` / `codex` / `opencode` / `pi` / `grok` / `omp` commands, with a line-snapshot fallback for ↑ history and Tab completion) plus 500ms output-activity polling, showing idle / working / error states.
...
- **oh-my-pi (omp) hook integration** — `omp` (can1357/oh-my-pi, a pi fork) has no shell-command hooks; its extension point is a TypeScript module loaded in-process by its Bun runtime. Registration therefore writes a self-contained extension, `miniterm.ts`, into `~/.omp/agent/extensions/` (honoring `PI_CODING_AGENT_DIR` / `PI_CONFIG_DIR` / `PI_PROFILE`), and the extension `fetch`es the local hook server directly from inside omp — no sidecar binary, and outside a Mini-Term terminal (no `MINITERM_PTY_ID`) it is a no-op. omp's lifecycle events are translated onto the Claude event names, so the hook server needs no changes: `agent_start` → `UserPromptSubmit`, `agent_end` → `Stop` (or `StopFailure` when the last assistant turn ended in error; a `stopReason` of `aborted` is reported as `Stop` with `reason: aborted` and lands as `Interrupt`, never as a bogus "task complete"), `tool_call` / `tool_result` → `PreToolUse` / `PostToolUse` (`PostToolUseFailure` on error), the `ask` tool → `Elicitation` / `ElicitationResult` (amber lamp while it waits for your answer), `tool_approval_requested` / `tool_approval_resolved` → `PermissionRequest` / `PreToolUse` or `PermissionDenied`, `auto_retry_start` → a retrying `Notification` (stays `ai-working`), auto-compaction → `PreCompact` / `PostCompact`, `session_switch` / `session_branch` → the old session ends with `reason: clear` and the new one starts, `session_shutdown` → `SessionEnd`. Only the interactive lead session reports (`ctx.mode === "tui"`): omp's subagents (the `task` tool) are separate sessions inside the same process that re-bind every extension factory, and reporting from them would announce the parent as "done" whenever a subagent finished. omp only scans the extensions directory at startup, so a running instance needs a restart or `/reload-plugins` after registering; Mini-Term rewrites the file to the current template on launch whenever it is registered, so upgrades carry over. Session logs (`~/.omp/agent/sessions/{encoded cwd}/{timestamp}_{session-id}.jsonl`) are parsed for the mobile mirror — located precisely by the hook-reported session id, covering user / assistant text, ask tool calls and their answers; the history panel, usage stats, and session branching stay off for omp; startup resume works through the hook-reported session id (`omp --resume <id>`).
...
- **File workbench** — Local and remote files opened from the tree live in main-area tabs alongside the terminal, where you view, edit, and save them: tree-sitter syntax highlighting (30+ languages) matched automatically by file type, basic indentation, find & replace (`Ctrl+F`); tab indentation is expanded to tab stops (4 columns each) for display, so Go and other tab-indented files no longer render flush against the left edge — the Git diff panel expands them the same way — and saving restores the original bytes line by line through an "expanded line → source line" map (only lines you actually edited get their leading indentation folded back into tabs; untouched lines are written back verbatim); `Ctrl+S` saves atomically (temp file + rename, so a bad write can't corrupt the file), and CRLF files round-trip with their original line endings so you never get a whole-file diff; closing with unsaved changes asks first, and external modifications reload silently when clean or show a notice bar when dirty; Markdown preview renders the unsaved draft live; syntax colors follow the theme skin. Remote files are read and written over SFTP: every save is compared against the baseline loaded earlier, and if someone changed the file on the server you can reload or force-overwrite; writes go through temp file + backup + rename with stale backups cleaned up automatically; a failed refresh only shows a banner instead of covering the editor; any remote file can also be downloaded locally. The tab bar supports bulk closing and horizontal scrolling: right-click a tab for "Close / Close others / Close to the right / Close to the left" (the pinned terminal tab is outside every close scope and gets no menu at all, so it can never be closed); a bulk close containing unsaved files asks once and lists the file names (more than five fold into "and N more"), and if the active tab is taken along, focus lands on the tab you right-clicked. Tabs are no longer squeezed — once they overflow the bar scrolls horizontally (the vertical wheel drives it too), a thin scrollbar surfaces along the bottom edge on hover and can be dragged directly, and switching, opening, or bulk-closing always pulls the target tab back into view, while the terminal tab stays pinned to the far left and never scrolls away.
- **Block-virtualized Markdown preview** — The preview splits a document into blocks and renders them one by one, laying out only the blocks near the viewport (`gpui::list` with 600px of overdraw), so scrolling a long document no longer re-lays out the whole thing every frame: on a measured 65KB document with 17 tables, main-thread CPU per frame drops from 151.6ms to 22.6ms, and a 226-item list goes from 145.3ms to 49.2ms. Scroll position is carried by the block list and restored from the logical scroll offset whenever the block count, the parse generation, or the viewport width changes — switching between source and preview, saving, or dragging the splitter all come back to a pixel-identical position; this also fixed the old full-document layout drawing heading blocks on top of their neighbors. The trade-offs are documented in the implementation: selections in blocks scrolled out of view are dropped, and the wheel step is computed from a fixed line height.
...
- **Changes / history in one view** — The Git panel stacks two collapsible sections: Changes on top and commit history below, with a draggable divider (clamped 15%–85%) and animated collapse / expand, remembering fold state and ratio for the session; a repo bar at the top of the panel switches repos via a dropdown (worktree entries marked ⎇), clicking the branch badge only switches which branch's history is shown (no checkout, highlighted when viewing a non-HEAD branch) and the badge itself has lost its fixed width cap — it sizes to the branch name and only truncates with an ellipsis plus a full-name tooltip when the repo bar really can't fit it (it shrinks later than the repo name, so long branch names are preserved first), refresh / Pull / Push sit on the same bar, and right-clicking the repo name opens it in a terminal or enters worktree management.
...
- **Upstream branch pill** — While viewing a local branch's history, its upstream (`origin/xxx`) is pinned as a dimmed pill on the commit it actually points at, next to the local branch name — so how far ahead you are, and whether your latest work has been pushed, is visible at a glance. Branch lists are now compared by name *and* target commit, so the pill follows along right after a commit or push instead of staying stuck on the old commit.
...
- **Branch info** — Local / remote branch lists, with upstream tracking info on local branches.
...
- **Multi-repo discovery** — Automatically scans all Git repos under the project directory (recursing 5 levels, skipping `node_modules` etc.), sorted by path so entry order doesn't drift with filesystem enumeration order. A repo's name is its leaf directory name, so monorepo layouts like `services/api` and `legacy/api` produce a run of identical-looking entries — each dropdown row therefore carries a dimmed second line with its **parent path** (identical leaf names always live under different parents, so this always tells them apart), omitted for repos sitting one level down. The selected repo in the repo bar gets the same dimmed parent prefix, with the full path on a tooltip.
...
- **Custom title bar** — The frameless window gets a self-drawn top bar: app name, version and project switcher on the left, window controls on the right, colored from the theme instead of the system's grey strip. Adapted to each platform's conventions:
  ...
  - A **global status light** used to sit right beside the project switcher, aggregating the most urgent state across every project. It was removed in v1.2.6 — the pill already carries a dot for the current project's state, and two colored dots side by side just made you work out which was which. "Jump to the session that needs you next" is not lost: a left click on the tray icon does exactly that, and each row of the pill's dropdown jumps within its own project.
...
- **Truncated text really gets an ellipsis** — gpui's `truncate()` only clips inside a flex item sized to its natural width, drawing no ellipsis, so text looked like it had simply been sliced off. The custom `mt_ui::TruncatedText` element instead truncates in prepaint, once the final width is known, and appends `…`; this batch of call sites was converted over and their containers given `min_w(0)` so they can actually be compressed: file-viewer breadcrumbs and Markdown image placeholders, directory group names and file paths in Git changes / diffs, worktree groups and the current dropdown item, project names / descriptions / ⎇ branch badges / remote-connection badges in the project list, search result paths and group file names, settings tabs and theme-pack names, SSH panel group headers and connection names, and project paths on pane preview cards.
...
| Tests | **1,782 Rust tests** (29 test targets) + relay-server protocol boundary tests |
...
```bash
# Workspace-wide Rust tests (29 test targets, 1,782 cases)
