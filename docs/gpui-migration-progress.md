...
- **底座换到 gpui-pre 0.3.5 + gpui-component 0.6.1**（2026-09-18，分支 `spike/gpui-pre`）：Zed 官方 `gpui` 0.2.2 后停更，组件库 0.6 起改依赖 huacnlee 上传的 Zed 快照 `gpui-pre`（zed@d89e9c2）。窗口/文字后端拆到了 `gpui_platform`，入口改 `gpui_platform::application()`，Linux 的 `x11/wayland` feature 挪到它上面。API 变更全是机械的：`window.focus(h, cx)` 多 `cx`；`ShapedLine::paint` 多 `TextAlign, Option<Pixels>`；`truncate_line` 多 `TruncateFrom` 且返回 `(SharedString, Cow<[TextRun]>)`；`paint_image(bounds, image_bounds, ..)` 自带 cover 裁剪；`AsyncApp::update` / `Entity::update(AsyncApp)` 不再返回 `Result`（App 退出时任务连同循环被丢弃）；`Bounds::contains` 右/下缘改开区间；`ClickEvent::Touch`、`ClipboardEntry::ExternalPaths`、`ImageFormat::{Ico,Pnm}`、`BoxShadow.inset`、`SyntaxColors.text_code_span` 新增。组件库侧：`Dialog::confirm()/alert()` 没了（自动页脚只剩 `AlertDialog`，其 `build_surface` crate 私有）→ `prompt::confirm_footer` 自绘页脚、按钮派发 `Confirm`/`Cancel` 动作走原路径；`InputState` 拆成 `InputState/TextareaState/EditorState`（提交框、代码编辑器各换一处）；`TextView::markdown(id, text)` 去掉 window/cx；`ScrollbarShow` → `ScrollbarMode`；`track_scroll` 按引用。语言包：0.6.1 内置 php（带 HTML 注入）/ lua（带 injections+locals），我们那两份覆盖删掉（再引 `tree-sitter-lua` 会双版本、C 符号重定义）；**内置 kotlin 的 highlights.scm 在它自己拉的 kotlin-sg 0.4.1 上编不过**（`Invalid node type "!is"`，组件库只 warn 后静默退成纯文本），我们的 kotlin 覆盖包保留。收益：md 预览打开延迟 1649ms → 481ms（0.6.1 高亮器按语言缓存 Query，同机同协议 debug 对照）；Markdown 链接处置整块补齐（见下一条）。
- ~~Markdown 链接三条处置整块缺~~ **已补回**（2026-09-18，随 gpui-pre 升级）：gpui-component 0.6 的 `TextView::on_link_click` 开了回调口。外链弹确认再开浏览器、`#锚点` 滚到标题所在块（ATX 标题按 GitHub slug 比对，原始 HTML `id="…"` 也认，围栏代码块里的 `#` 不算）、mailto 等其它协议直接交系统、本地文件**作为页签打开**（已开的切过去；本地来源先验文件在不在，缺了弹「链接指向的文件不存在」）。原版单 modal 里换文件的 `←` 历史栈不搬——GPUI 版文件本来就是页签。纯逻辑（`classify_link` / `resolve_local_href` / `heading_slug` / `block_has_anchor`）有单测；原版 `resolveLocalHref` 对 POSIX 根下相对链接会丢前导 `/` 的 bug 顺手修了。
...
- **P 批记档（搜索三连 + 快捷键让路）**：
  ...
  - `Dialog` 当自绘浮层用的三件套：`.p_0()`（默认 24px 内边距会把自画的分隔线切断）、`.close_button(false)`（它画 `IconName::Close`，0.5.1 无 svg 资产 → 空白）、聚焦输入框要 `window.defer`（`open_dialog` 会在之后把焦点抢到面板上）。**2026-09-19 补记**：应用入口已挂 `gpui_kit_assets::Assets`（`dfc5e1d`），`IconName::Close` 现在画得出来，「空白」这条判据作废；但 `prompt.rs` 的 `open_guarded_with_close` 仍统一 `.close_button(false)`，需要 ✕ 的弹窗走 `prompt::dialog_title` 自绘（✕ 画在标题行右端，上游那颗则是绝对定位到面板右上角、`p_0()` 之下落在 8,8），两颗都开就是重复 ×——故自绘 ✕ 保留，现状无重复 ×。
  ...
  - 遗留：SearchModal 点结果依赖 `FileViewerModal`（#29）未迁移，现退到外部编辑器打开；~~结果列表无虚拟化~~（**2026-09-19 已改** `gpui_component::v_virtual_list`：结果展平成 `ResultRow` 一维表 + `RowMetrics` 按当前字号/DPI 现算行高；两条硬坑——滚动句柄必须存视图上否则每帧清零、行根必须 `w_full` 否则 hover 底色只盖文字）；分组头无 sticky；`ProjectSwitcher` 面板高度按候选条数估算（`Dialog` 只吃固定高度，没有 `max-h` 语义）。
- **fork 批（c337716 后）记档**：家族面板每次悬停现拉无跨菜单缓存（会话极多时首展有后台扫描延迟，与原版同）；同 agent「全新会话误记成 fork 子会话」残余窗口仍在（磁盘边合并优先+首次身份即消费+pty 退出清登记三道压到最小，与原版同）；~~`AgentBranchCaps::resume_command` 是有意保留的无调用点代码~~（2026-09 W4 已删：能力位表连同续接/fork 模板、id 白名单收进 `mt_ai::agent`，历史面板、分支菜单、启动续接共用一套）；menu.rs 自绘子菜单展开期间每次重绘调渲染闭包——有状态面板必须懒建缓存（view_branches_menu_item 是范例）；「带 cwd 失败重试」刻意不搬（GPUI spawn_pane 不因目录失败返 None）。
...
- **polish 批（f244fd8 后）记档**：菜单选中项是视图状态非 DOM 焦点（「关闭还焦点」纪律的前提），高亮与 hover 同底色（原版两者共用同一条 CSS）；menuPopIn/pane 进场均不做 scale（gpui 无 transform，改尺寸会挪文字/触发字号反解），只淡入+位移；浮层进场期间 6px 负 margin 不参与 anchored 贴边测量，贴窗缘弹出前 160ms 可能 ≤6px 越界、落位即正（不修——要动 gpui 测量口径）；done-tag 过冲缩放只动水平内边距防行高抖；`modal::open_rename_pane` 顺带修了默认值全选（原版同走 showPrompt）；真机验收点：用户机器 reduce=on，更新圆点与 done-tag 应静止/瞬时，验动效需临时开系统「动画效果」。**2026-09-19 补记（gpui-pre 回归）**：gpui-pre 0.3.5 新增 `App::reduce_motion`，`with_animation` 在闸开着时直接跳终态、不再请求帧（`elements/animation.rs:407-414`）；gpui-component 0.6.2 的 `init` 会读同一个 `SPI_GETCLIENTAREAANIMATION` 把闸打开，于是 09-18 升级起抽屉滑入滑出、页签指示条、切终端左右推移、分屏竖直推移这些走 `with_animation` 的豁免类过渡在用户机器上全部冻结（走 `Transition::drive`/`request_animation_frame` 的菜单、pane 进场没坏）。修法：`motion::install(cx)` 在 `gpui_component::init` 之后 `cx.set_reduce_motion(false)` 接管那道粗粒度闸（上游 `reduce_motion.rs:24-28` 明示应用一旦写过就归应用），豁免面仍由 `mt_ui::motion` 的闸 + `TransitionSpec::respects_reduce` 表达；副作用是 gpui-component 自家的滚动条/对话框/tooltip 淡入淡出也回到 0.2.2 时代的照播。toast 滑入在 reduce 下瞬时本是原版口径（`TOAST_SLIDE_IN.respects_reduce`），**同日用户拍板改为豁免**（方向性位移不该压成瞬时），`TOAST_SLIDE_IN` 改 exempt 规格、`toast.rs` 去掉调用点的 reduce 分支恒挂 `with_animation`；`TAG_FADE_IN`（done-tag 过冲）仍过闸。
...
- **统计面板三处 UI 修复（2026-08-20，用户报）**——三条各自的上游事实值得记档：
  ...
  - ~~自定义日期框宽度不够~~ **已修**：`gpui_component::Input` 从给定宽度里逐层扣左右各 12 的 padding + 1px 边框 + `RIGHT_MARGIN`(10，给光标留的)，原来的 112 只剩 76px 可视；而它的文字是 **rem 定死的 14px**（`input_text_size` → `text_sm`），**不跟 `ui::font_px` 的 UI 字号缩放走**，等宽字族下 `2026-08-20` 要 84px。改 150 并补 `flex_none`（原来那两层可收缩，挤的时候还会更窄），另配自绘日历浮层 `date_picker.rs`——**不用 `gpui_component::time::DatePicker`**：它的日历图标与翻月箭头全走 `AssetSource`，0.5.1 的 crate 包里一个 svg 都没有（上游把 lucide 放在示例程序的资产目录），渲染出来是三块空白且编译期无感。顺带：`DeferredDraw` **不保存 `content_mask_stack`**，所以 `deferred` 浮层挂在抽屉式面板里也不会被抽屉裁掉，不必像 `menu.rs` 那样做成全局层。**2026-09-19 补记**：应用入口已挂 `gpui_kit_assets::Assets`（`dfc5e1d`），上游那三枚图标现在画得出来，「图标缺失」这条理由作废；但「`Input` 文字 rem 定死 14px（`input_text_size` → `text_sm`）、不跟 `ui::font_px` 缩放」这条在 0.6.2 仍成立（另加配色取 `cx.theme()`），换回上游 `DatePicker` 仍需先 spike。
  - ~~趋势图面积有锯齿~~ **已修（真几何 bug，不是抗锯齿）**：`chart.rs::band_top_edge` 把曲线采样点**逐顶点钳位**进渐变横带却不在穿带处插交点 —— 折线先钳顶点再连线 ≠ 折线被钳出来的形状。陡坡上两个相邻采样点跨好几条带时，每条带都把「本该只占一小截 x」的斜边摊到整段 x 上：同一列各带只填自己下半截，拼出来是十几条横纹（总面积仍是对的，所以肉眼就是「曲线下方一排锯齿」且色块溢到曲线上方）。修法是逐段求与 `lo`/`hi` 的交点、`t ∈ (0,1)` 才插。参考：Windows 侧 path 走 4×MSAA 渲进中间纹理再 resolve（`directx_renderer.rs`），缓坡上的 4 级覆盖度是 1px 级毛边，量级比这个小一个数量级。同批把 `samples_per_segment` 上限 8 → 24（7 天视图一格 70px，8 段折线拟合三次曲线在弯处看得出是折的；总点数照旧由 `MAX_CURVE_POINTS=600` 兜住）。**2026-09-19 补记**：横带方案整体作废——`paint_path` 从来就吃 `Background` 渐变（记档「只吃单色」是误判，DX11 路径片元着色器确实解 `gradient_color`），面积改成一条带 `linear_gradient` 的 path，`band_top_edge`/`gradient_bands` 已删，上面的交点插值逻辑随之不存在；顶端 stop 用 `area_peak_color` 按峰值高度反推，保证同一屏幕高度同色。
...
| Z | 壳层杂项 + Toast + 提示音（#30 + 细项） | titlebar-shell-misc.md §B/§C | 关窗确认=同步钩子返 false + 弹框 + force_close 标志再 remove_window；自建 toast.rs（gpui-component Notification 四条结构性缺口）；双音走内存合成 WAV + PlaySoundW(SND_MEMORY|SND_ASYNC)，Beep 会阻塞 UI 线程。**2026-09-19 对照 gpui-base 0.6.2 复核**：悬停暂停（`toast.rs` 的 `advance(now, paused)`，上游把悬停态当 `paused` 传进去）、条数可配（`NotificationSettings.max_items`）、位置尺寸可配（`placement` / `margins` / `width`）三条已修；仍缺去重（同 id 仍是**替换**，原版要「同项目已有就忽略」）与 × 常驻（还是 `invisible()` + `group_hover`）；图标缺失那条理由随入口挂上 `gpui_kit_assets::Assets`（`dfc5e1d`）作废 |
