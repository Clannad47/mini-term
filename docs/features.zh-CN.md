...
- **长文本截断带省略号** — gpui 的 `truncate()` 在自然宽度的 flex 项里只做裁剪、不画省略号，看上去像被生生切了一刀；自绘的 `mt_ui::TruncatedText` 改为在 prepaint 拿到最终宽度后再截断并补 `…`，这批位置统一换过来并给容器补 `min_w(0)` 让它真能被压缩：文件查看器面包屑与 Markdown 图片占位标签、Git 变更 / 差异的目录分组名与文件路径、Git 工作树分组与下拉当前项、项目列表的项目名 / 描述 / ⎇ 分支徽章 / 远程连接徽章、搜索结果路径与分组文件名、设置左侧页签与主题包名，SSH 面板分组标题与连接名、面板预览卡的项目路径
...
| 测试 | **1805 个 Rust 测试**（29 个测试目标）+ 中转服务端协议边界测试 |
