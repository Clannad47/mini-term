...
//! # 为什么全是自绘
//!
//! 三条现成的路都走不通,判据写在 [`vector`] 的模块注释里(一句话:`svg()` 只出
//! 单色 alpha 掩膜,多色的品牌/文件图标画不了;`img(ImageFormat::Svg)` 在
//! gpui 0.2.2 上红蓝互换)。自绘的额外好处是**几何是纯数据**,映射表和形状都能
//! 单测。(2026-09-19 补记:判据里原先还有一条「要宿主注册 asset source」——
//! mt-app 入口已挂 `gpui_kit_assets::Assets`,那条作废,余下两条不变。)
//!
