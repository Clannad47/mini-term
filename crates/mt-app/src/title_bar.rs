...
//!
//! 与 M 批边条、P 批菜单同源的几条硬伤:高度写死 34px / 按钮 34px 宽(原版
//! 32 / 46);配色取 `cx.theme()` 而不是壳的 [`crate::ui`];布局是两段式,塞不下
//! 「品牌 / 版本 / 胶囊 / 状态灯 / 中段空白」这套五段结构;三键与 logo 的字形
//! 照抄原版(见下面 `ICON_*` 的形状表),与上游那套不是一回事。
//! **2026-09-19 补记**:原先第一条记的是「图标走 `IconName` → SVG 资产,本仓
//! 没注册 `AssetSource`,渲染成空白且编译期无感」—— 入口已挂
//! `gpui_kit_assets::Assets`(见 `main.rs` 的 `with_assets`),该判据作废,
//! 余下三条仍然成立。
