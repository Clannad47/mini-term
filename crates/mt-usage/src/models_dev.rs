//! 模型价格表:`https://models.dev/api.json` → 归一 → 24h 磁盘缓存。
//! 对应 `src/utils/modelPricing.ts`(206 行),逐规则移植。
//!
//! ```text
//! ensure_pricing(dir, now, fetch)
//!   ├─ 手工裸表 {dir}/model-pricing.json          ← 恒优先、无 TTL(用户自己放的)
//!   ├─ 新鲜缓存 {dir}/model-pricing.cache.json    ← version==2 且 24h 内
//!   ├─ fetch()(拉网 → normalize)→ 非空 → 写缓存
//!   └─ 过期/旧版缓存兜底 → 都没有才 Err
//! ```
//!
//! 原先整份住在 mt-app 的 `pricing.rs`,2026-09 把归一 / 缓存 / 降级链路挪到这里
//! (与查价表 [`crate::PricingTable`] 同一个 crate,单测不再陪 GPUI 链接)。**网络
//! 那一跳留在 mt-app**(`pricing::fetch_models_dev`,HTTP 客户端是 gpui 依赖树里的
//! zed-reqwest):[`ensure_pricing`] 的 `fetch` 由调用方注入,本 crate 不带 HTTP
//! 客户端;拉回来的 JSON 交给 [`normalize_pricing_table`] 归一。
//!
//! # 为什么建键要做「全序择优」
//!
...
//! 优先的规则根本不触发,碰撞被推迟到 `PricingTable` 才发生,而那边
...
///
/// 必须与账本侧查价的 `pricing::canonical()` **逐规则一致**(含先后顺序:
/// 先取 `/` 后段再剥 `@`),否则这里择优出来的键在账本侧会二次塌陷,择优白做。
/// 住在 mt-app 时这是一份逐字副本、靠注释提醒两侧同步;挪进同一个 crate 后
/// 直接调那一份,不再有两份可漂。
pub fn canonical_model_key(name: &str) -> String {
    crate::pricing::canonical(name)
}
...
/// 可序列化的单模型价格。[`ModelPrice`] 只 derive 了 `Deserialize`
...
// ─── 装配(阻塞,调用方必须丢 background executor) ──────

/// 取一份可用的价格表。**整体阻塞**(`fetch` 是一次 HTTPS GET),调用方丢
/// `cx.background_executor()`。
///
/// `fetch` = 拉网 + [`normalize_pricing_table`],由调用方注入(mt-app 的
/// `pricing::fetch_models_dev`);只有前两级都没命中时才会被调用。
/// 拉网成功时顺带写缓存;手工表 / 新鲜缓存命中即瞬时返回,不碰网络。
pub fn ensure_pricing(
    dir: &Path,
    now_ms: i64,
    fetch: impl FnOnce() -> Result<PricingMap, String>,
) -> Result<(PricingMap, PricingSource), String> {
    ...
    let result = resolve_pricing(manual, cache, now_ms, fetch)?;
...
mod tests {
    ...
    fn 择优比较器不平局() {
        ...
        assert_eq!(
            compare_pricing_candidates(
                &candidate("aaa", "m", false, false),
                &candidate("bbb", "m", false, false)
            ),
            Ordering::Greater
        );
        ...
        assert_eq!(
            compare_pricing_candidates(
                &candidate("p", "aaa", false, false),
                &candidate("p", "bbb", false, false)
            ),
            Ordering::Greater
        );
        // 完全相同才允许平局
    ...
    fn 旧版本缓存不当新鲜值但可兜底() {
        ...
        let (t, src) =
            resolve_pricing(None, Some(old.clone()), now, || Ok(table_of("net", 3.0))).unwrap();
    ...
    #[test]
    fn 手工裸表与缓存信封各读各的() {
        let dir = std::env::temp_dir().join("mt-usage-pricing-split-test");
        ...
        // 手工表原样还在
        assert!(
            load_manual_pricing(&dir)
                .unwrap()
                .contains_key("claude-opus-5")
        );
    ...
    #[test]
    fn 空手工表当没有() {
        let dir = std::env::temp_dir().join("mt-usage-pricing-empty-test");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
