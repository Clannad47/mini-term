...
//!
//! 「A」类里有制表符 `─│┌┐`(U+2500..)、希腊/西里尔字母、`±×÷`、`●○★→` ——
//! Claude Code 的对话框边框全是制表符,消息前缀用的 `●` 也在里面,而它排版靠的
//! string-width 按 1 列算;整类改宽等于把每个 TUI 骨架撑爆。所以只收「CJK 字体画成
//! 全角、又不会出现在 TUI 骨架里」的**枚举符号**块,表在 [`FORCED_WIDE_RANGES`],
//! 要扩就加一行。
//!
//! # 落格手法:借道占位字符
//!
...
mod tests {
    ...
    /// 反例:同属 ambiguous 类却**不能**改宽的——TUI 骨架用的制表符、Claude Code 的
    /// 消息前缀 `●`、箭头、希腊字母;以及本来就是 W 的 CJK / 0 宽的组合符号。
    #[test]
    fn 其它字符原样放行() {
        for c in ['─', '│', '╭', '●', '○', '★', '→', 'α', 'Я', '±', 'a', ' '] {
    ...
    /// 对照组:制表符与 `●` 仍是 1 列——Claude Code 的边框与消息前缀不能被撑开。
    #[test]
    fn 制表符与圆点仍是一列() {
        let e = TerminalEmulator::new(TermSize::new(20, 3));
