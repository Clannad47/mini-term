...
static PROJECT_SWITCHER_EN: &[(&str, &str)] = &[
    ...
    ("placeholder", "Type a project or group name…"),
...
static SETTINGS_ZH: &[(&str, &str)] = &[
    ...
    ("aiHook.footer", "注册后所选 CLI 会在事件发生时通知 Mini-Term · 各家的配置文件互不相干，可单独装/卸 · oh-my-pi 装的是扩展文件，正在运行的 omp 需重启或执行 /reload-plugins · 未注册时自动使用进程轮询检测"),
    ("aiHook.noTargetSelected", "至少勾选一个注入目标"),
    ("aiHook.register", "注册所选 Hook"),
    ("aiHook.registering", "注册中..."),
    ("aiHook.serverDesc", "通过 Claude Code / Codex / Grok / oh-my-pi 的 hook 系统获取精确的 AI 状态，替代进程轮询"),
    ...
    ("aiNotification.footer", "以上通知在 AI 任务（Claude / Codex / Grok / oh-my-pi）完成时触发"),
    ...
    ("shortcuts.switchProject", "快速切换项目"),
...
static SETTINGS_EN: &[(&str, &str)] = &[
    ...
    ("aiHook.footer", "Once registered, the selected CLIs notify Mini-Term when events occur · Each CLI's config file is independent, so each can be installed or removed on its own · oh-my-pi gets an extension file; a running omp needs a restart or /reload-plugins · Falls back to process polling when not registered"),
    ...
    ("aiHook.serverDesc", "Get precise AI status via the Claude Code / Codex / Grok / oh-my-pi hook systems, replacing process polling"),
    ...
    ("aiNotification.footer", "The notifications above fire when an AI task (Claude / Codex / Grok / oh-my-pi) completes"),
    ...
    ("shortcuts.switchProject", "Quick-switch project"),
